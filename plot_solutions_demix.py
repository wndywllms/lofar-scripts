...
def solplot_ampphase_target(parmdb, imageroot, target, refstationi, norm_amp_lim=False, median_amp=False, plot_international=False):
    ...
    'Gain:1:1:Phase:RS508HBA'
    stationsnames = np.array([name.split(':')[-2] for name in names])
    stationsnames = np.unique(stationsnames)
    if not plot_international:
        stationsnames = np.array([name for name in stationsnames if name[0] in ['C','R'] ])
    Nstat = len(stationsnames)
    ...
    refstation = stationsnames[refstationi]
    times = soldict['DirectionalGain:1:1:Real:{s}:{s1}'.format(s=refstation,s1=target)]['times']
    times = times-times[0]
    tunit='s'
    if times[-1] > 60:
        tunit='min'
    ...
    real11_ref = soldict['DirectionalGain:1:1:Real:{s}:{s1}'.format(s=refstation,s1=target)]['values']
    real00_ref = soldict['DirectionalGain:0:0:Real:{s}:{s1}'.format(s=refstation,s1=target)]['values']
    imag11_ref = soldict['DirectionalGain:1:1:Imag:{s}:{s1}'.format(s=refstation,s1=target)]['values']
    imag00_ref = soldict['DirectionalGain:0:0:Imag:{s}:{s1}'.format(s=refstation,s1=target)]['values']
    ...
    for istat, station in enumerate(stationsnames):
        ...
        real11 = soldict['DirectionalGain:1:1:Real:{s}:{s1}'.format(s=station,s1=target)]['values']
        real00 = soldict['DirectionalGain:0:0:Real:{s}:{s1}'.format(s=station,s1=target)]['values']
        imag11 = soldict['DirectionalGain:1:1:Imag:{s}:{s1}'.format(s=station,s1=target)]['values']
        imag00 = soldict['DirectionalGain:0:0:Imag:{s}:{s1}'.format(s=station,s1=target)]['values']
    ...
    fp.savefig(imageroot+"_"+target+"_phase.png",dpi=100)
    
    
    fa.savefig(imageroot+"_"+target+"_amp.png",dpi=100)
    return
...
if __name__ == "__main__":   
    ...
    if args.phaseonly:
        ...
        target = 'Target'
        solplot_ampphase_target(parmdb, imageroot, target, refstation, norm_amp_lim=norm_amp_lim, median_amp=median_amp, plot_international=plot_international)
        target = '3c298'
        solplot_ampphase_target(parmdb, imageroot, target, refstation, norm_amp_lim=norm_amp_lim, median_amp=median_amp, plot_international=plot_international)
